...
%%TODO: section into Model, Algorithm, etc.
...
Building off the previously described methods, we construct a SVM classifier using word vectors as features, in addition to extra features, including:
\begin{itemize}
\item Entities discovered with NER (Named Entity Recognition)
\end{itemize}
...
\section{Preprocessing step: normalization of the data}
...
\subsection{More Semantically Coherent Phrases} We saw a 10\% inrease in model accuracy when we extended the wordVec algorithm over phrases as well.  However, in our implementation, these phrases are only ngrams, which do not in general correspond very well to semantic units.  Therefore, we could probably get a significant improvement by parsing each tweet into a parse tree first with a standard semantic parser, and learning vector representations for the phrases output by that.

TODO: is this not what RAE does?

TODO: future direction: 1. not vec, 2. throw away similarity values below a certain threshold
